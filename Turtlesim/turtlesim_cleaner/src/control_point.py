...
class TurtleBot:
    ...
    def __init__(self, tolerance):
        ...
        self.pid_linear_vel = PID(tolerance, 2, 0.001, 1,output_limits=(-100,100))
...
# corresponding y axis values
                ...
                # while (self.steering_angle(goal_pose) - self.pose.theta) >= angle_tolerance:
                #     vel_msg.angular.x = 0
                #     vel_msg.angular.y = 0
                ...
                # print(self.pid_ang_vel.output_limits)
                # self.pid_ang_vel.output_limits=(-60,60)
                
                # vel_msg.angular.z = 0
                # self.velocity_publisher.publish(vel_msg)
                # self.pid_ang_vel.tunings=(10,0.01,1.5)
                
                # self.pid_ang_vel.set_output_limits((-100, 100))
                while self.euclidean_distance(goal_pose) >= distance_tolerance:
                    ...
                    y1.append(vel_msg.linear.x)
                    y2.append(vel_msg.angular.z)
                    x1.append(i)
                    i+=1
                    y3.append(self.pose.x)
                    y4.append(self.pose.y)
...
if __name__ == '__main__':
    try:
        ...
        x = TurtleBot(distance_tolerance)
        x.move2goal(distance_tolerance)
