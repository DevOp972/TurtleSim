...
class TurtleBot:
    ...
    def __init__(self):
        ...
        self.velocity_publisher = rospy.Publisher('/turtlebot1/cmd_vel',
                                                  Twist, queue_size=100)
        self.velocity_pub_circle = rospy.Publisher('/turtlebot2/cmd_vel',
                                                   Twist, queue_size=10)

        # A subscriber to the topic '/turtlebot/pose'. self.update_pose is called
        # when a message of type Pose is received.
        self.pose_subscriber = rospy.Subscriber('/turtlebot1/pose',
    ...
    def move2goal(self):
        ...
        vel_msg_circle = Twist()

        while not rospy.is_shutdown():

            while(True):
                vel_msg_circle.linear.x = 2.75
                ...
                while self.euclidean_distance(goal_pose) >= distance_tolerance:
                    vel_msg_circle.linear.y = 0
                    vel_msg_circle.linear.z = 0
                    vel_msg_circle.angular.x = 0
                    vel_msg_circle.angular.y = 0
                    vel_msg_circle.angular.z = 1
                    self.velocity_pub_circle.publish(vel_msg_circle)
