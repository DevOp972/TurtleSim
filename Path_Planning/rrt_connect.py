...
class RrtConnect:
    def __init__(self, s_start, s_goal, step_len, goal_sample_rate, iter_max, x_range, y_range):
...
def main():
    en = env.Env()
    x_start = (en.start_pts[2][0], en.start_pts[2][1])  # Starting node
    print("\n\n", en.start_pts)
    x_goal = (en.goal_pts[0][0], en.goal_pts[0][1])  # Goal node
    print("\n\n", en.goal_pts)

    rrt_conn = RrtConnect(x_start, x_goal, 0.8, 0.05,
                          5000, en.x_range, en.y_range)
